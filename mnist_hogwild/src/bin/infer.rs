//! MNIST Hogwild CNN inference example.
//!
//! Loads a trained MNIST Hogwild CNN model and classifies synthetic 28x28 images.
...
use mnist_hogwild::MnistCNN;

fn main() {
    println!("=== MNIST Hogwild CNN Inference ===");
    println!();

    // Load trained model
    let bytes = std::fs::read("mnist_hogwild_model.safetensors")
        .expect("Model file not found. Run the training first: cargo run --bin mnist_hogwild");
    let sd = load_state_dict(&bytes).unwrap();
    let mut model = MnistCNN::from_state_dict(&sd);
    model.set_eval();
    println!("Model loaded from mnist_hogwild_model.safetensors");
